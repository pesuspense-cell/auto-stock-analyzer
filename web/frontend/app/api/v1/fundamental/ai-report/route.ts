// POST /api/v1/fundamental/ai-report — AI 재무 리포트를 jobs 큐에 인서트(비동기).
// 무거운 LLM 리포트 생성은 Python 워커(interactive 레인)가 수행한다.
...
export async function POST(req: Request): Promise<NextResponse<JobEnqueued | ApiError>> {
  ...
  try {
    ...
    const params = { ticker, useLlm: Boolean(body?.use_llm ?? body?.useLlm ?? true) };

    const { data, error } = await supabase
      .from("jobs")
      .insert({ user_id: user.id, kind: "fundamental_ai", status: "pending", params })
      .select("id,status")
      .single();
    if (error || !data) {

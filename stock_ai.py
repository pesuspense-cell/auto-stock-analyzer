...
# ─── 펀더멘털 데이터 ──────────────────────────────────────────────────────────
# get_fundamental_data / get_naver_fundamentals / _is_krx_ticker は
# src.fundamental (Provider 패턴) 으로 이전됨.

def get_fundamental_data(ticker: str) -> dict:
    """src.fundamental.get_fundamental_data 위임 (하위 호환)."""
    from src.fundamental import get_fundamental_data as _fn
    return _fn(ticker)
...
def get_insider_trades_sec(ticker: str, days: int = 90) -> pd.DataFrame:
    """SEC EDGAR Form 4 내부자 거래 (미국 주식 전용) — DARTProvider 위임."""
    from src.providers.dart_provider import DARTProvider

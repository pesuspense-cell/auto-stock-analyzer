...
from stock_ai import (
    ...
    analyze_news_sentiment_llm, get_hybrid_signal,
...
st.caption(f"업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  |  종목: **{sname}** (`{ticker}`)")

# ─── 데이터 로드 ──────────────────────────────────────────────────────────────
with st.spinner(f"{ticker} 데이터 분석 중..."):
    data = _stock_data(ticker, period)

if data.empty:
    st.error(f"'{ticker}' 데이터를 불러올 수 없습니다. 티커를 확인해주세요.")
    st.stop()
    ...
    for a in _alerts:
        ...
        if uid not in already_shown:
            if a["score"] >= 4:
                st.toast(
                    f"🟢 **{a['name']}** 강력 매수!\n"
                    f"신호: {a['label']} ({a['score']:+d}점)  |  {a['price']:,.0f}  {a['chg']:+.2f}%",
                    icon="🔺",
                )
            elif a["score"] <= -4:
                st.toast(
                ...
                st.toast(
                    f"🟡 **{a['name']}** 매수 신호\n"
                    f"신호: {a['label']} ({a['score']:+d}점)  |  {a['price']:,.0f}",
                    icon="🔔",
                )
            elif a["score"] <= -3:
                st.toast(
...
with tab1:
    ...
    with col_chart:
        ...
        fig = make_subplots(
            ...
            subplot_titles=("가격 (캔들·이평선·볼린저밴드)", "거래량", "RSI (14)", "MACD (12·26·9)"),
        ...
        if "BB_Upper" in data.columns:
            fig.add_trace(go.Scatter(
                ...
                line=dict(color="rgba(158,158,158,0.7)", width=1, dash="dot"), showlegend=False,
            ), row=1, col=1)
            fig.add_trace(go.Scatter(
                x=data.index, y=data["BB_Lower"], name="BB하단",
                line=dict(color="rgba(158,158,158,0.7)", width=1, dash="dot"),
                fill="tonexty", fillcolor="rgba(158,158,158,0.08)", showlegend=False,
            ), row=1, col=1)
        ...
        if "Volume_MA20" in data.columns:
            fig.add_trace(go.Scatter(
                x=data.index, y=data["Volume_MA20"], name="거래량MA20",
        ...
        if "RSI" in data.columns:
            ...
            for level, color in [(70, "rgba(239,83,80,0.4)"), (30, "rgba(66,165,245,0.4)")]:
        ...
        # 서브플롯 타이틀 폰트 — 작게 해서 차트와 겹침 방지
        fig.update_annotations(font_size=11, font_color="#9e9e9e")
    ...
    with col_sig:
        ...
        <div class="signal-box" style="background:{bg};">
            <div style="font-size:2.2rem; font-weight:bold; color:{fc};">{badge} {label}</div>
            <div style="font-size:1.1rem; color:#aaa; margin-top:6px;">종합 점수: <b style="color:{fc};">{h_score:+.1f}점</b></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div style="background:#1e2130;border-radius:8px;padding:10px 14px;margin-top:6px;font-size:0.88rem;">
            <div style="display:flex;justify-content:space-between;">
                <span style="color:#aaa;">📊 기술 점수 (70%)</span>
                <b style="color:{tc};">{tech_score:+d}</b>
            </div>
        ...
        for r in signals.get("reasons", []):
            if any(k in r for k in ["매수", "반등", "상승", "골든"]):
                st.success(r, icon="🔺")
            elif any(k in r for k in ["매도", "과열", "하락", "데드"]):
                st.error(r, icon="🔻")
            else:
                st.info(r, icon="🔷")
        ...
        indicator_map = {
            "RSI": "RSI", "MACD": "MACD",
            "BB상단": "BB_Upper", "BB하단": "BB_Lower",
            "5일선": "SMA_5", "20일선": "SMA_20",
        }
        last_row = data.iloc[-1]
        for lbl, col in indicator_map.items():
            if col in data.columns:
                val = last_row[col]
                if pd.notna(val):
                    fmt = f"{float(val):.1f}" if lbl == "RSI" else f"{float(val):,.2f}"
                    st.caption(f"**{lbl}:** {fmt}")
...
with tab3:
    ...
    if rec_df is not None and not rec_df.empty:
        buy_n     = int((rec_df["신호점수"] >= 1).sum())
        neutral_n = int((rec_df["신호점수"] == 0).sum())
        sell_n    = int((rec_df["신호점수"] <= -1).sum())

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("분석 종목",    f"{len(rec_df)}개",
                  help="이번 분석에서 데이터를 불러온 종목 수입니다.")
        m2.metric("🟢 매수 신호", f"{buy_n}개",
                  help="기술 신호 점수가 +1점 이상인 종목 수. 점수가 높을수록 매수 강도가 강합니다.")
        m3.metric("⚪ 중립/관망", f"{neutral_n}개",
                  help="기술 신호 점수가 0점인 종목 수. 뚜렷한 방향성이 없는 상태입니다.")
        m4.metric("🔴 매도 신호", f"{sell_n}개",
                  help="기술 신호 점수가 -1점 이하인 종목 수. 점수가 낮을수록 매도 강도가 강합니다.")

        st.divider()
        st.markdown("### 📋 종목별 상세 분석표")

        def _row_style(row):
            s = row["신호점수"]
            if s >= 3:  return ["background-color:#1b5e20"] * len(row)
            if s >= 1:  return ["background-color:#2e7d32"] * len(row)
            if s <= -3: return ["background-color:#b71c1c"] * len(row)
            if s <= -1: return ["background-color:#c62828"] * len(row)
            return [""] * len(row)

        disp = ["종목명", "현재가", "등락률(1일)%", "신호", "신호점수",
                "예상수익률(%)", "변동성(%)", "모멘텀(20일)%", "샤프지수"]
        styled = (
            rec_df[disp].style
        ...
        for _, row in rec_df.iterrows():
            s     = row["신호점수"]
            color = "#66bb6a" if s >= 2 else ("#ef5350" if s <= -2 else "#ffa726")
        ...
        fig_sc.add_hline(y=0, line_color="white", line_dash="dash", opacity=0.25)
        ...
        # 신호 점수 막대
        st.markdown("### 🏆 신호 점수 순위")
        fig_rank = go.Figure(go.Bar(
            x=rec_df["종목명"],
            y=rec_df["신호점수"],
            marker_color=["#66bb6a" if val >= 0 else "#ef5350" for val in rec_df["신호점수"]],
            text=rec_df["신호"],
            textposition="outside",
        ))
        fig_rank.update_layout(
            height=320, template="plotly_dark",
            yaxis_title="신호 점수", margin=dict(t=10, b=60),
            xaxis_tickangle=-30,
        )
        ...
        | 법칙 | 출처 | 매수 조건 | 매도 조건 |
        |------|------|-----------|-----------|
        | RSI  | 기술적 분석 | < 35 과매도 | > 65 과매수 |
        | MACD | 기술적 분석 | 골든크로스 | 데드크로스 |
        | 볼린저밴드 | 기술적 분석 | 하단 이탈 | 상단 돌파 |
        | 이동평균선 | 스탠 와인스타인 | 5일선 상향 돌파 | 5일선 하향 이탈 |
        | 거래량 | CANSLIM (오닐) | 급증 + 상승 | 급증 + 하락 |
        """)
...
# ══════════════════════════════════════════════════════════════════════════════
with tab5:
    st.subheader(f"🏛️ {sname} 펀더멘털 & 기관 분석")

    with st.spinner("펀더멘털 데이터 로딩 중..."):
        fund_info = _fundamental(ticker)
    ...
    # ── 펀더멘털 점수 패널 ────────────────────────────────────────────────────
    with col_f1:
        st.markdown("### 📊 투자법칙 기반 펀더멘털 점수")

        fs    = fund_score_data.get("fund_score", 0)
        flbl  = fund_score_data.get("fund_label", "N/A")
        total = signals.get("score", 0) + fs

        if fs >= 3:
            fbg, ffc = "#1a3a2a", "#81c784"
        elif fs <= -2:
            fbg, ffc = "#3e1616", "#ef9a9a"
        else:
            fbg, ffc = "#1e2130", "#bdbdbd"

        st.markdown(f"""
        <div class="signal-box" style="background:{fbg};">
            <div style="font-size:1.6rem; font-weight:bold; color:{ffc};">{flbl}</div>
            <div style="font-size:1rem; color:#aaa; margin-top:4px;">
                펀더멘털 점수: <b style="color:{ffc};">{fs:+.1f}</b>
                &nbsp;|&nbsp; 종합(기술+펀더멘털): <b>{total:+.1f}</b>
            </div>
        </div>
        """, unsafe_allow_html=True)

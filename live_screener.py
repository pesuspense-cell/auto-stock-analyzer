...
backtest.py 핵심 로직 + 리스크 관리 모듈 완전 계승:
  ...
  ▸ [v2] ATR 동적 손절선(버퍼 상향): 야수 max(진입가 − 3.0×ATR, 진입가 × 0.88) / 디펜스 진입가 − 3.0×ATR
...
MAX_POSITION_PCT:     float = 0.15   # [v2] 단일 종목 평가액 상한 = 총자산의 15% (자산 보호 하드캡)
SL_FALLBACK_PCT:      float = 0.90   # ATR 미산출 시 손절 폴백 (-10%)
ATR_SL_MULT_BULL:     float = 3.0    # [v2] 야수 손절 ATR 배수 (2.5→3.0, 노이즈 칼손절 방지)
BULL_SL_FLOOR_PCT:    float = 0.88   # [v2] 야수 손절 하한 (-8%→-12%)
ATR_SL_MULT_BEAR:     float = 3.0    # [v2] 디펜스 손절 ATR 배수 (2.0→3.0)
SL_SLIPPAGE_PCT:      float = 0.01   # [개편4] 손절 체결 보수 슬리피지 (-1%)
NULIM_PRIORITY_BONUS: float = 8.0    # [개편2] 눌림목 전략 우선순위 가산점
MAX_GROSS_EXPOSURE:   float = 0.45   # [v3] 총 주식노출 상한 = 총자산의 45% (초과 시 비중축소 권고) — MDD 주 방어
...
def calc_buy_plan(
    ...
    backtest.py _resolve_stop_loss + _calc_position_qty 완전 동일:
      손절가 = 야수 max(close−3.0×ATR, close×0.88) / 디펜스 close−3.0×ATR
    ...
    if signal == "SMA_GOLDEN_CROSS":
        sl = max(close - ATR_SL_MULT_BULL * atr, close * BULL_SL_FLOOR_PCT) if has_atr else close * SL_FALLBACK_PCT
        tp = close * 1.25                                   # +25% 고정 익절
    else:  # DRY_VOLUME_NULIM 등
        sl = (close - ATR_SL_MULT_BEAR * atr) if has_atr else close * SL_FALLBACK_PCT
...
def main() -> None:
    ...
    gross_pct   = held_value / total_asset if total_asset > 0 else 0.0
    print(f"  💼 총자산 평가액 : {total_asset:>15,.0f} 원 (현금 {cash:,.0f} + 주식 {held_value:,.0f})")
    print(f"  📊 총 주식노출   : {gross_pct:>14.0%}  (상한 {MAX_GROSS_EXPOSURE:.0%})")
    # [v3] 노출 상한 초과 시 비중축소(리밸런싱) 권고
    cap_value        = MAX_GROSS_EXPOSURE * total_asset
    rebalance_excess = max(0.0, held_value - cap_value)
    if rebalance_excess > 0:
        print(f"  ⚠️ 총노출 상한 초과 — 약 {rebalance_excess:,.0f}원 비중축소(평가액 큰 종목부터) 권장")
    print()
    ...
    if buy_locked:
        ...
        cur_n = n_pos
        running_invested = held_value          # [v3] 누적 노출 추적 (45% 상한 준수)
        for c in candidates:
            if cur_n >= max_positions:
                print(f"  최대 보유 종목 수({max_positions}개) 도달 — 추가 매수 지침 생략")
                break
            if running_invested >= cap_value:
                print(f"  총노출 상한({MAX_GROSS_EXPOSURE:.0%}) 도달 — 추가 매수 지침 생략")
                break
            plan = calc_buy_plan(
                candidate        = c,
                cash             = cash,
                total_asset      = total_asset,
                current_invested = running_invested,
                n_positions      = cur_n,
            ...
            if plan:
                ...
                cur_n += 1
                running_invested += plan["alloc_cash"]
